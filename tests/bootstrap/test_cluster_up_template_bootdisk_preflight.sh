...
echo "\$url" >>"$curl_log"
if [[ "\$url" == *"/qemu/9000/config" ]]; then
  printf '{"data":{"bootdisk":"scsi0","ide2":"local-zfs:vm-9000-cloudinit,media=cdrom"}}' >"\$output_file"
  printf '200'
  exit 0
fi
printf '{}' >"\$output_file"
...
if PATH="$fake_bin:$PATH" TFVARS_FILE="$tfvars_file" bash bootstrap/scripts/cluster-up.sh "$env_file" >"$output_file" 2>&1; then
  echo "cluster-up.sh unexpectedly succeeded with a malformed template that lacks its boot disk" >&2
  exit 1
fi

grep -q 'template VM 9000 on node pve-1 is malformed' "$output_file"
grep -q 'missing boot disk device scsi0' "$output_file"

if [[ -s "$terraform_log" ]]; then
  echo "cluster-up.sh should fail before invoking terraform when the template boot disk is missing" >&2
  exit 1
fi
