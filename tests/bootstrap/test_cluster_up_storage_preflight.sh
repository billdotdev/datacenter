...
echo "\$url" >>"$curl_log"
if [[ "\$url" == *"/storage/local-lvm/status" ]]; then
  printf '{"errors":"storage '\''local-lvm'\'' does not exist"}' >"\$output_file"
  printf '500'
  exit 0
fi
printf '{}' >"\$output_file"
...
if PATH="$fake_bin:$PATH" TFVARS_FILE="$tfvars_file" bash bootstrap/scripts/cluster-up.sh "$env_file" >"$output_file" 2>&1; then
  echo "cluster-up.sh unexpectedly succeeded when the Proxmox storage is missing" >&2
  exit 1
fi

grep -q "storage local-lvm on node pve-1" "$output_file"
grep -q "does not exist" "$output_file"

if [[ ! -s "$curl_log" ]]; then
  echo "cluster-up.sh should check the Proxmox storage before invoking terraform" >&2
  exit 1
fi

if [[ -s "$terraform_log" ]]; then
  echo "cluster-up.sh should fail before invoking terraform when the Proxmox storage is missing" >&2
  exit 1
fi
